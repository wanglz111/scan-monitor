...
class Exercises:
    ...
    def split_msg(self, dict):
        ...
        return "发送方:{send}\n\n to:{recive}。\n\n 方式：{method} \n\n 价值：{value} BNB。\n\n [debank]({link}) \n [区块transfer]({transfer}) \t [区块hash]({hash_link}) \n\n 时间：{time}". \
            format(method=dict['method'], transfer=transfer_link, hash_link=bscscan_hash_link, link=debank_link,
                   value=float(dict['value']) / self.wei, time=self.stampTransformTime(int(dict['time'])),
                   send=dict['from'], recive=dict['to'])
    ...
    def split_msg_eth(self, dict):
        ...
        return "发送方:{send}\n\n to:{recive}。\n\n 方式：{method} \n\n 价值：{value} ETH。\n\n [debank]({link}) \n [区块transfer]({transfer}) \t [区块hash]({hash_link}) \t [opensea]({open_link}) \n\n 时间：{time}". \
            format(method=dict['method'], transfer=transfer_link, hash_link=bscscan_hash_link, link=debank_link,
                   open_link=opensea_link, value=float(dict['value']) / self.wei,
                   time=self.stampTransformTime(int(dict['time'])), send=dict['from'], recive=dict['to'])
    ...
    def split_msg_ftm(self, dict):
        ...
        return "操作方:{send}\n\n 操作:{recive}。\n\n 价值：{value} FTM。\n\n [debank]({link})\n [区块transfer]({transfer}) \n [区块hash]({hash_link}) \n\n 时间：{time}".format(
            transfer=transfer_link, hash_link=bscscan_hash_link, link=debank_link,
            value=float(dict['value']) / self.wei, time=self.stampTransformTime(int(dict['time'])), send=cur_from,
            recive=cur_to)
    ...
    def split_msg_matic(self, dict):
        ...
        return "操作方:{send}\n\n 操作:{recive}。\n\n 价值：{value} Matic。\n\n [debank]({link})\n [区块transfer]({transfer}) \n " \
               "[区块hash]({hash_link}) \n\n 时间：{time}".format(transfer=transfer_link, hash_link=bscscan_hash_link,
                                                             link=debank_link, value=float(dict['value']) / self.wei,
                                                             time=self.stampTransformTime(int(dict['time'])),
                                                             send=cur_from, recive=cur_to)
    ...
    async def send_message(self, final_text):
        ...
        async with bot:
            await bot.send_message(text=final_text, chat_id=group_chat_id)
    ...
    def get_recent_tx(self, address, rotate_count=0):
        ...
        # apikey_list = [self.apikey_ETH, self.apikey_BSC, self.apikey_MATIC]
...
if __name__ == "__main__":
    ins = Exercises()
    asyncio.run(ins.send_message("start"))
    while 1:
        logger.info("start")
        for key in oxdata:
